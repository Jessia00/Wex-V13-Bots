...
module.exports = { name: "voicemute", aliases: ["vmute", "sesmute"],  category: "Penal", desc: "Kullanıcıyı Ses Kanallarında Susturma Komutu",
    ...
    if(!message.member.permissions.has("8") && !Server.voiceMuteStaff.some(rol => message.member.roles.cache.has(rol))) return message.reply(AutoReply.YetersizYetki).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    ...
    if (User.roles.cache.has(Server.vmutedRole)) return message.reply(AutoReply.VmuteActive).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    ...
    if (Settings.voiceMuteLimit > 0 && voiceMuteLimit.has(message.author.id) && voiceMuteLimit.get(message.author.id) == Settings.voiceMuteLimit) return message.reply(`${AutoReply.voiceMuteLimit}`), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    ...
    message.reply(`${client.emojis.cache.find(x => x.name === "ramal_vmuted") || "Emoji Bulunamadı"} ${User} (\`${User.user.tag}\` - \`${User.id}\`) isimli kullanıcı, **${Reason}** sebebiyle ${message.author} tarafından \`${muteSure}\` süresince ses kanallarında susturuldu. (\`Ceza Numarası #${count}\``).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_tik")}`)
