...
module.exports = { name: "mute", aliases: ["cmute", "chatmute"], category: "Penal", desc: "Kullanıcıyı Metin Kanallarında Susturma Komutu",
    ...
    if(!Reason) return message.reply(AutoReply.SebebBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    if (!User) return message.reply(AutoReply.UyeBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    if (!Time) return message.reply(AutoReply.ZamanBelirt).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    if (User.roles.cache.has(Server.mutedRole)) return message.reply(AutoReply.CmuteActive), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    if (Settings["Bot.Owner"].includes(User.id)) return message.reply(AutoReply.BotSahibi), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    ...
    if (Settings.chatMuteLimit > 0 && chatMuteLimit.has(message.author.id) && chatMuteLimit.get(message.author.id) == Settings.chatMuteLimit) return message.reply(`${AutoReply.chatMuteLimit}`).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    ...
    if (User.roles.highest.position >= message.member.roles.highest.position && !Settings["Bot.Owner"].includes(message.author.id)) return message.reply(AutoReply.YetersizYetki).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_carpi")}`)
    ...
    message.reply(`${client.emojis.cache.find(x => x.name === "ramal_muted") || "Emoji Bulunamadı"} ${User} (\`${User.user.tag}\` - \`${User.id}\`) isimli kullanıcı, **${Reason}** sebebiyle ${message.author} tarafından \`${muteSure}\` süresince metin kanallarında susturuldu. (\`Ceza Numarası #${count}\``).then(e => setTimeout(() => e.delete().catch(() => { }), 10000)), message.react(`${client.emojis.cache.find(x => x.name === "ramal_tik")}`)
